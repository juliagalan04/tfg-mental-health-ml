...
OUTPUT_DIR = "../outputs_daily_stress_qcut"
...
# --------------------------------------------------
# 2. Create target from DAILY_STRESS using balanced bins
# --------------------------------------------------

df["DAILY_STRESS"] = pd.to_numeric(df["DAILY_STRESS"], errors="coerce")

df = df.dropna(subset=["DAILY_STRESS"]).copy()
...
class_counts = df["Stress_Category"].value_counts()
class_props = df["Stress_Category"].value_counts(normalize=True)
...
plt.title("Class Distribution - Stress Category")
...
columns_to_drop = ["DAILY_STRESS", "Stress_Category"]

# IMPORTANTE ELIMINARLA
if "WORK_LIFE_BALANCE_SCORE" in df.columns:
    columns_to_drop.append("WORK_LIFE_BALANCE_SCORE")
...
plt.title("Model Comparison by Macro F1-score - Stress Prediction")
plt.xlabel("Model")
