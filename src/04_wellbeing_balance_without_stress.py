...
OUTPUT_DIR = "../outputs_without_daily_stress"
...
# --------------------------------------------------
# 4. Features and target (WITHOUT DAILY_STRESS)
# --------------------------------------------------

columns_to_drop = ["WORK_LIFE_BALANCE_SCORE", "Balance_Category"]

if "DAILY_STRESS" in df.columns:
    columns_to_drop.append("DAILY_STRESS")
else:
    print("\nWARNING: 'DAILY_STRESS' column was not found. Check the exact column name.")
...
plt.title("Model Comparison by Macro F1-score (Without DAILY_STRESS)")
plt.xlabel("Model")
...
disp.plot()
plt.title(f"Confusion Matrix - {best_model_name} (Without DAILY_STRESS)")
plt.tight_layout()
...
if hasattr(clf, "coef_"):
    ...
    plt.title(f"Top {top_n} Important Features - {best_model_name} (Without DAILY_STRESS)")
